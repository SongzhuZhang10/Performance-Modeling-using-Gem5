...
Check binary path with all default optional arguments:
build/X86/gem5.opt configs/tutorial/part1/two_level.py tests/test-progs/hello/bin/x86/linux/hello
build/X86/gem5.opt configs/tutorial/part1/two_level.py

Check user-defined caches' sizes
build/X86/gem5.opt configs/tutorial/part1/two_level.py --l2_size=1MB --l1d_size=128kB

Check default caches' sizes
build/X86/gem5.opt configs/tutorial/part1/two_level.py
"""
...
# import the names from the caches.py file made by us into the namespace
from caches import *
...
print(f"Path of the binary: {default_binary}")
...
# Display the actual path of the binary file
print(f"Actual path of the binary: {options.binary}")
...
# Hook the CPU's cache ports up to the l2bus
...
# Connect the L2 cache to the membus which will be connected to mem controller later.
...
# Allow the system to read and write memory.
